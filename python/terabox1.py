...
class TeraboxFile():
    ...
    def getMainFile(self) -> None:
        ...
        url: str = f'https://www.1024terabox.com/api/shorturlinfo?app_id=250528&shorturl=1{self.short_url}&root=1'
    ...
    def getChildFile(self, short_url, path:str='', root:str='0') -> list[dict[str, any]]:
        ...
        url = 'https://www.1024terabox.com/share/list?' + '&'.join([f'{a}={b}' for a,b in params.items()])
        req : object = self.r.get(url, headers=self.headers, cookies={'cookie':''}).json()
...
class TeraboxLink():
    ...
    def __init__(self, fs_id:str, uk:str, shareid:str, timestamp:str, sign:str, js_token:str, cookie:str) -> None:
        ...
        self.r : object = requests.Session()
        self.headers : dict[str,str] = headers
        self.result : dict[str,dict] = {'status':'failed', 'download_link':{}, 'streaming_link': ''}
    ...
    def generate(self) -> None:
        try:
            ...
            url : str = 'https://www.1024terabox.com/share/download?' + '&'.join([f'{a}={b}' for a,b in params.items()])
            req : object = self.r.get(url, cookies={'cookie':self.cookie}).json()

            if not req['errno']:
                direct_url = req['dlink']
                self.result['download_link'].update({'url_1': direct_url})
                ...
                streaming_params = {
                    ...
                    'jsToken': self.dynamic_params['jsToken'],
                    'esl': '1',
                    'isplayer': '1',
                    'ehps': '1',
                    'clienttype': '0',
                ...
                streaming_url = 'https://www.terabox.app/share/extstreaming.m3u8?' + '&'.join([f'{a}={b}' for a,b in streaming_params.items()])
                self.result['streaming_link'] = streaming_url
...
class Test():
    ...
    def file(self) -> None:
        ...
        url = 'https://www.1024terabox.com/wap/share/filelist?surl=cmi8P-_NCAHAzxj7MtzZAw' #-> Test File (Zip)
